...
更新日期：2026-06-15  
项目样本：华住项目 / 上海江桥  
当前阶段：v0.7 数据库测试工具包已公开发布，并已通过本机 PostgreSQL 真实 smoke test；下一步进入 Excel 导入闭环。
...
- [x] 安装并确认 `psql` 客户端。
- [x] 安装并确认本机 PostgreSQL 16 测试环境。
- [x] 运行 `node scripts/run-v07-db-test.mjs`。
- [x] 修复 v0.7 seed SQL 的 PostgreSQL 语法和 `jsonb` 类型问题。
- [x] 完成本机真实数据库 smoke test。
- [x] 生成第一份数据库 smoke test 结果记录，作为 v0.8 Excel 导入前置证据。
- [ ] Docker Desktop / Colima 作为可选容器环境，后续网络稳定时再补。
